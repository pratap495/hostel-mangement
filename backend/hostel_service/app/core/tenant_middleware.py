...
# Context variable to hold active tenant database session for current thread
tenant_session_var: contextvars.ContextVar = contextvars.ContextVar("tenant_session")

# In-memory dictionary cache of SQLAlchemy Engine instances (Task 3.1)
engine_cache: Dict[str, any] = {}

class CentralTenantRegistry(Base):
    """Minimal SQLAlchemy model matching the Central DB tenant mapping registry."""
...
def get_tenant_db():
    """Dependency to retrieve the active request thread-safe tenant session."""
    try:
...
class TenantRoutingMiddleware(BaseHTTPMiddleware):
    """Dynamic context routing middleware across isolated databases (Task 3.1)."""
    
    async def dispatch(self, request: Request, call_next) -> Response:
        path = request.url.path
        
        # Bypass authentication/tenant checks on system endpoints
        if path in ["/health", "/api/v1/rooms/docs", "/api/v1/rooms/openapi.json"]:
        ...
        # 2. Query Central DB connection details
        ...
        if not engine_str:
            ...
            try:
                ...
                # Assemble connection URI (maps to Postgres container in hostelmint-net network)
                ...
                logger.info(f"Engine cached successfully for tenant hostel: {hostel_id}")
            except Exception as e:
                logger.error(f"Error reading tenant registry details: {e}")
                return Response(
        ...
        # 3. Spawn tenant database session and bind to active context
        ...
        try:
            ...
            session.rollback()
            logger.error(f"Transaction rollback executed due to: {e}")
            raise e
        finally:
            session.close()
