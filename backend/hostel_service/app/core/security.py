...
def decode_access_token(token: str) -> dict:
    """Decode and validate a JWT token against configuration algorithms."""
    try:
...
def get_current_user(token: HTTPAuthorizationCredentials = Depends(reusable_oauth2)) -> dict:
    """FastAPI security dependency resolving and validating incoming bearer tokens."""
...
def require_owner_or_admin(current_user: dict = Depends(get_current_user)):
    """Enforce Role-Based Access Control (RBAC) mapping to Owners and Super Admins."""
